...
namespace Skeleton
{
    class DisplaySkeleton
    {
        ...
        {
            ShowMenu();
            //Plan: So here the user must enter his first and last name and then we should check
            //if they are in the database,after that we must get the corresponding ID.
            //check supervisorControler and EmployeeController
            //find ID by two names and then assign it to a variable
            var worker = "";//find worker by two names.
            //if the worker is a supervisor then make the superID equal to the current
            //same for bearer
            //change if-s for the check
